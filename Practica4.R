...
color=c("darkslategray1","darkorchid3")
...
# 1.- Creación de un vector de color
color=c("burlywood", "coral1", "darkslategray2")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "brown4", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="darkslategrey", fill="deeppink1")+
  ggtitle("Histograma")+
...
GD2<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c("beige","deeppink4","darkseagreen2"))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()
